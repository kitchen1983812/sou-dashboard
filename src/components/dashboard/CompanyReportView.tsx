...
const STATUS_COLS = [
  ...
  { key: "諸事情", label: "諸事情により受入不可", status: [STATUS.CANNOT_ACCEPT] },
...
const AREA_COLORS: Record<string, { bg: string; text: string }> = {
  東京: { bg: "bg-red-100", text: "text-red-700" },
  神奈川: { bg: "bg-blue-100", text: "text-blue-700" },
  千葉: { bg: "bg-green-100", text: "text-green-700" },
  埼玉: { bg: "bg-yellow-100", text: "text-yellow-700" },
  認可: { bg: "bg-purple-100", text: "text-purple-700" },
};
const DEFAULT_AREA_COLOR = { bg: "bg-gray-100", text: "text-gray-600" };

function getAreaColor(area: string) {
  for (const [key, color] of Object.entries(AREA_COLORS)) {
    if (area.includes(key)) return color;
  }
...
interface PrevData {
  byNursery: Map<string, { total: number; enrolled: number; enrollmentRate: number; counts: Record<string, number> }>;
  total: number;
  enrolled: number;
  enrollmentRate: number;
  totalCounts: Record<string, number>;
...
function computeCompanyReport(inquiries: Inquiry[]): CompanySummary[] {
  ...
  companyMap.forEach((nurseryMap, company) => {
    ...
    STATUS_COLS.forEach((col) => { totalCounts[col.key] = 0; });
    let companyTotal = 0;
    let companyEnrolled = 0;

    nurseryMap.forEach((inqs, nurseryName) => {
      const counts: Record<string, number> = {};
      STATUS_COLS.forEach((col) => { counts[col.key] = 0; });
      ...
      rows.push({
        ...
        total,
        enrolled,
        enrollmentRate: total > 0 ? Math.round((enrolled / total) * 10000) / 100 : 0,
        counts,
      });
    });
    ...
    result.push({
      ...
      enrollmentRate: companyTotal > 0 ? Math.round((companyEnrolled / companyTotal) * 10000) / 100 : 0,
      totalCounts,
    });
  });
...
function computePrevData(inquiries: Inquiry[]): Map<string, PrevData> {
  ...
  for (const r of reports) {
    const byNursery = new Map<string, { total: number; enrolled: number; enrollmentRate: number; counts: Record<string, number> }>();
...
function getEnrollmentBg(rate: number): string {
  if (rate >= 20) return "bg-purple-200";
  if (rate >= 15) return "bg-purple-100";
  if (rate >= 10) return "bg-purple-50";
  if (rate >= 5) return "bg-indigo-50";
  return "";
}
...
function YoYRate({ current, prev }: { current: number; prev: number | undefined }) {
  if (prev === undefined) return null;
  const diff = current - prev;
  const arrow = diff > 0 ? "\u2191" : diff < 0 ? "\u2193" : "";
  const color = diff > 0 ? "text-red-400" : diff < 0 ? "text-blue-400" : "text-gray-400";
  ...
  return (
    ...
    >
      <span className="inline-flex items-center gap-0.5">
        {label}
        <span className={`text-[9px] ${active ? "text-red-600" : "text-gray-300"}`}>
...
function CompanyTable({
  ...
  const colMaxes = useMemo(() => {
    ...
    STATUS_COLS.forEach((col) => {
      m[col.key] = Math.max(...summary.rows.map((r) => r.counts[col.key] || 0), 1);
    });
    return m;
  }, [summary]);
      ...
      <table className="w-full text-xs border-collapse">
        ...
        <tbody>
          {sortedRows.map((row) => {
            ...
            const nameColor =
              row.enrollmentRate < brandRate - threshold ? "text-red-600 font-semibold" :
              row.enrollmentRate > brandRate + threshold ? "text-green-600 font-semibold" :
              "text-gray-700";
            ...
            return (
              ...
              >
                <td className={`px-2 py-1.5 truncate max-w-[150px] ${nameColor}`}>
                  {row.nurseryName}
                </td>
                <td className="px-1 py-1.5">
                  <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-medium ${areaColor.bg} ${areaColor.text}`}>
                    {row.area}
                  </span>
                </td>
                ...
                <td className={`px-1 py-1.5 text-center ${COL_W} ${getEnrollmentBg(row.enrollmentRate)}`}>
                  {row.enrollmentRate.toFixed(1)}%
                  <YoYRate current={row.enrollmentRate} prev={prevNursery?.enrollmentRate} />
                </td>
                {STATUS_COLS.map((col) => {
        ...
        <tfoot>
          ...
          <tr className="bg-gray-100 text-gray-500">
            ...
            {STATUS_COLS.map((col) => {
              const pct = summary.total > 0
                ? ((summary.totalCounts[col.key] / summary.total) * 100).toFixed(1)
                : "0.0";
              return (
                <td key={`pct-${col.key}`} className={`px-1 py-1 text-center ${COL_W}`}>
                  {pct}%
                </td>
              );
            })}
          ...
          <tr className="bg-red-50 font-bold border-t-2 border-red-300">
            ...
            <td className={`px-1 py-1.5 text-center text-red-800 ${COL_W}`}>
              ...
              <YoYRate current={summary.enrollmentRate} prev={prevCompany?.enrollmentRate} />
            </td>
            {STATUS_COLS.map((col) => (
              <td
                key={`total-${col.key}`}
              ...
              >
                ...
                <YoY current={summary.totalCounts[col.key]} prev={prevCompany?.totalCounts[col.key]} />
              </td>
            ))}
          </tr>
        </tfoot>
