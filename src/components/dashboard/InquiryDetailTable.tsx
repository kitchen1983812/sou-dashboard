				...
				<table className="w-full text-xs">
					...
					<tbody>
						{pageData.map((inq, i) => (
							<tr
								key={inq.id || `${page}-${i}`}
								className={`border-b border-gray-100 hover:bg-gray-100 transition-colors ${i % 2 === 1 ? "bg-gray-50/50" : ""}`}
							>
								...
								<td className="px-2 py-1.5 text-gray-700">{inq.name || "-"}</td>
								<td className="px-2 py-1.5 text-gray-700">
									{inq.status || "未対応"}
								</td>
					...
					<tfoot>
						<tr className="border-t-2 border-gray-300">
							<td
								colSpan={4}
								className="px-2 py-1.5 font-semibold text-gray-700 text-center"
							>
								総計
							</td>
