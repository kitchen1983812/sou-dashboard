...
export default function WeeklyReportView({
  ...
  const isCurrentWeek =
    getWeekStart(today).getTime() === selectedWed.getTime();

  // --- 週範囲 ---
  const thisTuesday = useMemo(
    () => getWeekEnd(selectedWed),
    [selectedWed]
  );
  const prevWed = useMemo(
    () => getPrevWeekStart(selectedWed),
    [selectedWed]
  );
  const prevTuesday = useMemo(
    () => getWeekEnd(prevWed),
    [prevWed]
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {recentWeeks.map((wed) => {
          ...
          const isCurrent =
            getWeekStart(today).getTime() === wed.getTime();
          return (
            <button
              key={wed.toISOString()}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                <tr className="bg-red-50 font-bold border-t-2 border-red-300">
                  ...
                  >
                    {(() => {
                      const total = ageAreaRows.reduce(
                        (s, r) => s + r.diff,
                        0
                      );
                      return total > 0
