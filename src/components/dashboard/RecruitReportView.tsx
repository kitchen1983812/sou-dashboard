...
export default function RecruitReportView({
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.nursery) data = data.filter((a) => a.nursery === filters.nursery);
  ...
  const funnelRate = (n: number) =>
    funnel.total > 0
      ? `${Math.round((n / funnel.total) * 1000) / 10}%`
      : "0%";
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        ...
        <div className="flex items-end gap-1">
          {[
            { label: "応募", value: funnel.total, color: "bg-gray-400" },
            {
              label: "書類選考通過",
            ...
            { label: "合格", value: funnel.passed, color: "bg-green-400" },
            {
              label: "内定承諾",
      ...
      {/* 不採用カード */}
      <div className="bg-white border border-gray-200 rounded-lg p-3 w-fit">
...
function CrosstabTable({
  ...
  const maxVal = Math.max(
    ...data.flatMap((row) =>
      statusCols.map((s) => (row[s] as number) || 0)
    ),
    1
  );
